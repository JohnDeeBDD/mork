...
<div id = "mork-comments-container">
	...
	if ( have_comments() ) : ?>
		<h2 class="comments-title">
			<?php
				...
				if ( 1 === $comment_count ) {
					printf(
						/* translators: 1: title. */
						esc_html_e( '%1$s Comment', 'marinate' ),
						'<span>' . get_the_title() . '</span>'
					);
				} else {
					printf( // WPCS: XSS OK.
						/* translators: 1: comment count number, 2: title. */
						esc_html( _nx( '%1$s Comments', '%1$s Comments', $comment_count, 'comments title', 'marinate' ) ),
		...
		<nav id="comment-nav-above" class="navigation comment-navigation">
			<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'marinate' ); ?></h2>
			<div class="nav-links">

				<div class="nav-previous"><?php previous_comments_link( esc_html__( 'Older Comments', 'marinate' ) ); ?></div>
				<div class="nav-next"><?php next_comments_link( esc_html__( 'Newer Comments', 'marinate' ) ); ?></div>
	...
	if ( ! comments_open() && get_comments_number() && post_type_supports( get_post_type(), 'comments' ) ) : ?>
		...
		<p class="no-comments"><?php esc_html_e( 'Comments are closed.', 'marinate' ); ?></p>
	<?php
	endif;
    ...
    $comment_arg = array(
        ...
        'comment_field' =>  '<p class="comment-form-comment"><label for="comment">'.__('Comment', 'marinate').'</label><textarea id="comment" class="form-control" name="comment" placeholder="' .esc_attr('Leave your comment here...', 'marinate'). '" cols="45" rows="10" required="required" aria-required="true">' .
            '</textarea></p>',

        'must_log_in' => '<p class="must-log-in">' .
        sprintf( /* translators: %s: login url */
        __( 'You must be <a href="%s">logged in</a> to post a comment.', 'marinate' ), 
        wp_login_url( apply_filters( 'the_permalink', get_permalink() ) )
        ) . '</p>',

        'logged_in_as' => '<p class="logged-in-as">' .
        sprintf( /* translators: %s: user profile url */	
        __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'marinate' ), 
